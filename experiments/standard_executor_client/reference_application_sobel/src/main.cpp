...
class SobelNode : public rclcpp::Node
{
  public:
    ...
    : Node("sobel_node")
    {
      ...
      myfile.open ("sobel.csv");
      myfile << "Sobel" << ";" <<std::endl;
      RCLCPP_INFO(this->get_logger(), "SobelNode started");
      publisher_ = this->create_publisher<sensor_msgs::msg::Image>("/image_raw", 10);
      subscription_ = this->create_subscription<sensor_msgs::msg::Image>("filtered", 10, std::bind(&SobelNode::topic_callback, this, _1));
      timer_ = this->create_wall_timer( std::chrono::milliseconds(2000), std::bind(&SobelNode::timer_callback, this));


      RCLCPP_INFO(this->get_logger(), "File name for publishing image is : %s", filename_.c_str());

      image_ = cv::imread("/home/christian/sobel.jpg", 1);
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<SobelNode>());
  rclcpp::shutdown();
  return 0;
}
