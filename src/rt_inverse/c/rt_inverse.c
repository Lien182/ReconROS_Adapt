...
THREAD_ENTRY() {
	...
	std_msgs__msg__UInt32 * inputmsg = (std_msgs__msg__UInt32*)data;
	start = clock();


	printf("[Inverse] got data: %x \n", inputmsg->data);
	float t_p2b_alpha = fitofl((inputmsg->data >> 17) & 0x3fff, 14, 6);
	float t_p2b_beta  = fitofl((inputmsg->data >> 3) & 0x3fff, 14, 6);
	...
	rinverse_output_msg->data = ((v_s_aj_l_mina << 21) | (leg << 18) | 0);
	end = clock();
